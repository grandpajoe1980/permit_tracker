...
function CustomerRequestTriageQueue({
  ...
  return <Card className="mb-6 border-amber-200 bg-amber-50/60"><CardHeader><CardTitle className="text-lg font-black text-[#00284d]">Customer intake queue</CardTitle><p className="text-sm text-slate-600">Review the complete request before routing it to one or more teams. All submitted requests are shown.</p><Input aria-label="Search intake requests" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search confirmation, title, submitter, or agency" className="bg-white" /></CardHeader><CardContent className="space-y-3">{visible.length > 0 ? visible.map((request) => <div key={request.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-amber-200 bg-white p-3"><div><p className="text-sm font-black text-[#00284d]">{request.confirmationNumber} · {request.title}</p><p className="mt-1 text-xs text-slate-600">{request.requestType.replaceAll("_", " ")} · {request.submittedByName}</p></div><Button type="button" size="sm" onClick={() => onTriage(request)} className="bg-[#00284d] text-xs font-bold">Review and route</Button></div>) : <p className="text-sm text-slate-600">No submitted requests match “{query}”. Clear the search to see all intake.</p>}</CardContent></Card>;
}
