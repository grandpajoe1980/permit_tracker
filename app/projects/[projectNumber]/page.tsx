...
export default async function ProjectRoute({ params }: { params: Promise<{ projectNumber: string }> }) {
  ...
  const projectQuery = isUuid
    ? client.from("projects").select("id, number, name, description, location, status").eq("id", projectNumber)
    : client.from("projects").select("id, number, name, description, location, status").or(`number.eq."${projectNumber}",id.eq."${projectNumber}"`);
