...
export default async function WorkstreamRoute({ params }: { params: Promise<{ projectNumber: string; workstreamId: string }> }) {
  ...
  if (!user.user) return <main className="mx-auto max-w-4xl space-y-4 p-8"><h1 className="text-2xl font-bold">Sign in required</h1><p className="text-slate-600">Sign in to view this authorized workstream. After signing in, return to this URL to continue.</p><Link href={`/?view=project&workstream=${encodeURIComponent(workstreamId)}&projectId=${encodeURIComponent(projectNumber)}`} className="inline-flex text-sm font-bold text-teal-800 hover:underline">Open PATH sign-in</Link></main>;
  ...
  redirect(buildShellPath("project", workstream.id, undefined, undefined, project.number));
}
