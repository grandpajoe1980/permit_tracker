...
export function TriageRoutingDialog({
  ...
  return <div ref={dialogRef} tabIndex={-1} className="fixed inset-0 z-50 flex items-end justify-center bg-[#00284d]/60 p-3 sm:items-center" role="dialog" aria-modal="true" aria-labelledby="triage-routing-title" aria-describedby="triage-routing-description">
    <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl">
      <div className="border-b border-slate-100 bg-slate-50 p-5"><p className="text-xs font-black uppercase tracking-[0.18em] text-teal-800">Intake routing review</p><h2 id="triage-routing-title" className="mt-1 text-xl font-black text-[#00284d]">{request.confirmationNumber} · {request.title}</h2><p id="triage-routing-description" className="mt-1 text-sm text-slate-600">Choose the deliberate outcome for this request. Suggestions never create work until the database confirms your choice.</p></div>
