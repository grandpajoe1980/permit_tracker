...
export function AdminDirectory({
  ...
  const filteredUsers = uniqueUsers.filter((user) => `${user.name} ${user.email} ${user.organization} ${user.agency}`.toLowerCase().includes(query.trim().toLowerCase()));
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-slate-50 p-3"><div><p className="text-sm font-black text-[#00284d]">People and access</p><p className="mt-1 text-xs text-slate-600">{teamUsers.length} visible user profile{teamUsers.length === 1 ? "" : "s"} · includes people without an active organization role.</p></div><Input aria-label="Search people and access" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search name, agency, or email" className="w-full max-w-sm bg-white sm:w-80" /></div>
    {teamUsers.length === 0 && <p className="py-2 text-xs text-slate-500 italic">Hydrating user directory profiles…</p>}
    {teamUsers.length > 0 && filteredUsers.length === 0 && <p className="rounded-lg border border-dashed border-slate-300 p-4 text-sm text-slate-600">No people match that search.</p>}
    {filteredUsers.map((user) => {
      ...
      return <div key={user.id} className="rounded-lg border border-slate-200 p-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <select aria-label={`Role for ${user.name}`} value={user.roleId} onChange={(event) => onRoleChange(user.id, event.target.value as RoleId)} className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-bold text-slate-800 sm:w-auto">
    ...
    <section className="border-t border-slate-200 pt-6" aria-labelledby="assignment-groups-heading">
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
        <div className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3">
          ...
          <div className="flex flex-col gap-2 sm:flex-row"><Input aria-label="New team description" value={newGroup.description} onChange={(event) => setNewGroup((current) => ({ ...current, description: event.target.value }))} placeholder="What does this queue handle?" /><button type="button" onClick={createGroup} disabled={!newGroup.name.trim() || !newGroup.orgCode.trim()} className="w-full shrink-0 rounded-md bg-teal-700 px-3 py-2 text-xs font-black text-white disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto">Create team</button></div>
