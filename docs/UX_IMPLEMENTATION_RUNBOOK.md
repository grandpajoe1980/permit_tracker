...
| CP9 — workflow authoring | Verified | Authenticated live draft from canonical v1 preserved the published stage contract, passed server validation, published as v3, and a new customer-request triage created a workstream pinned to v3 while existing demo instances remained pinned to their prior versions; browser authoring coverage also exercised save/reopen/validation/publish |
...
| Supabase migration consistency | Verified | `npx supabase migration list --linked` reports all 62 local/remote versions equal through `20260909140000`; linked `db push --dry-run` reports `upToDate: true` with no pending migrations, seeds, or roles |
...
- CP9 workflow evidence: the live draft RPC was first found to clone legacy `workflow_stages`, producing non-canonical owner codes in a newly published v2. A forward migration, `20260909140000_preserve_published_workflow_stage_contract.sql`, now copies the authoritative `workflow_version_stages` contract, including owners, assignment-group defaults, RFI/hold behavior, dependencies, and tasks, with legacy rows retained only as fallback. An authenticated Joe session created v3 from canonical `workflow-version-spaceport-request-v1`; all five stage owner codes matched the source, `rpc_validate_workflow_draft` returned `{ valid: true, errors: [] }`, and `rpc_publish_workflow_version` published v3. An authenticated Alex submission `PATH-CP9-REQ-20260909-V3` followed by Joe triage created `PATH-CP9-WS-20260909-V3` pinned to v3. Existing `PATH-DEMO-WS-COAST` remains pinned to v1 and the earlier v2 instance remains pinned to v2, proving version pinning. The authenticated browser authoring flow had already exercised draft creation, stage edit, save/reopen, invalid-duration rejection, correction, and publish; the live RPC repair then closed the contract-copy defect without editing published history.
