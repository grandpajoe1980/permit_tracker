...
- Deployment signal: GitHub reports a successful `Vercel` status for the final branch commit, but direct Vercel deployment lookup is rejected by the connected account with HTTP 403 scope authorization. The deployed SHA and production smoke test therefore remain open.
