...
| CP9 — workflow authoring | Implemented—unverified | Fixture/source coverage exists for draft/reopen/validation/publish/version pinning; a fresh authenticated live draft → publish → new instance → old-instance-pinned story remains |
...
- Still open: complete customer/staff/admin browser journey coverage; counts-versus-rows and completion downstream effects; the live workflow draft/publish/version-pinning story; full keyboard/touch coverage beyond the exercised drawer/focus path; final exact document/notification/coordination cross-checks where not yet rerun on the current SHA; and two consecutive final user-story/consistency review passes. No release acceptance claim is made from source, RPC, partial live evidence, or branch synchronization alone.
