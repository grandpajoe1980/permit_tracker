...
| R4 — real-stage schedule | Verified | September 9 flat-row revision supersedes expandable stages/tasks. Six focused tests and authenticated Chrome/Firefox Scenario 4 pass locally and on production `28e7c81fbd00958be6be8e81b56f8f306a1c4c9d`. One project/workstream row contains individually colored stages, with concurrent intervals sharing the track and undated stages explicitly outside the calendar. Today now moves with the actual date range, not a hardcoded marker over unrelated dates. |
...
2. Render exactly one ungrouped horizontal row per project/workstream, with individual stage colors and no expansion controls or aggregate stage-count bar. Concurrent intervals share the same track without covering each other. Stage details are available on hover/focus; unscheduled stages are clearly outside the date grid. This supersedes the earlier expandable stage/task rows. Preserve canonical task access through the chronological list and record pages.
