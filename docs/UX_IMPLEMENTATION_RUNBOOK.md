...
| R4 — real-stage schedule | Verified | September 9 lane-direction follow-up: a workflow never rises after stepping down; same-day handoffs continue right on the same lane, while actual duration overlaps staircase lower. Short terminal bars can place one readable label in free space to their right; intermediate/same-date successors reserve that space. Focused tests and local/deployed Chrome/Firefox Scenario 4 pass; deployed application SHA `ea587109e273ffa6ac3ffaf8185b7d37f1b02a78`. |
...
- Lane-direction follow-up: stage layout now carries the highest lane forward so the timeline only proceeds right or steps down; no later stage rises. The overlap rule treats matching handoff dates as sequential, matching adjacent visual bars. Short-stage labels may escape only from a terminal bar into unused right-hand space; same-date successors reserve that space to avoid text collisions. Focused layout tests cover overlap chains, no upward return, and same-day handoff. Local authenticated Scenario 4 passes in Chrome and Firefox with runtime checks for nondecreasing lane offsets and external label geometry; screenshot inspected. Build and explicit secret scan pass. This follows the React rendering review by keeping the layout projection pure and derived during render.
- Deployed lane-direction acceptance: production health reports `ea587109e273ffa6ac3ffaf8185b7d37f1b02a78`; authenticated Chromium and Firefox Scenario 4 pass 2/2 against that deployment. This documentation-only checkpoint records the result.

- Staircase follow-up: `lib/gantt-stage-layout.ts` separates display forecasting from persisted dates. Tests cover adjacency, 10-day fallback, zero propagation, overlap chains, partial dates, and input immutability. The workflow editor now retains/accepts explicit zero; mandatory statutory minimum validation remains enforced. Supabase/Postgres review preserved all authorization and audit checks, and the React review kept the projection derived without effects or extra requests.
- Deployed staircase acceptance: production health reports `32ad43f3ab0d8bee059114f1971a96526f3a818f`; authenticated Chromium and Firefox Scenario 4 pass 2/2 against that deployment. This documentation-only checkpoint records the result.
- Follow-up verification: 467 serial tests, 459 passed, 0 failed, 8 environment-gated skips; focused schedule and workflow tests pass. Build and explicit secret scan pass; changed-file lint reports 0 errors and 11 pre-existing warnings. Authenticated local Chromium and Firefox Scenario 4 pass, asserting every bar is 36px high, staircase offsets are nonzero, the parent accommodates the bars, and Today, focus tooltips, and 390px reflow remain correct. Screenshot inspected.
- Zero-duration server contract: migration `20260909115717_allow_unscheduled_stage_duration.sql` applied; **70/70 migration versions match**, and dry-run has no pending changes. An authenticated-role administrator SQL transaction creates a temporary draft, saves zero and reads it back, rejects negative duration, rejects zero below a statutory minimum, then rejects an unauthenticated call; the entire probe rolls back. This is database contract evidence, not a browser save/reopen claim. Existing workflow versions and histories are not rewritten.

- September 9 flat Gantt follow-up: removed expansion controls, nested stage/task rows, and aggregate stage-count bars; retained the existing palette and canonical links. Compact project/workstream labels, stage hover/focus details, explicitly unscheduled colored segments, inclusive one-day bars, and bounded month columns replace the old layout. No workflow dates, published versions, or migrations were changed for this repair.
- Deployed follow-up evidence: production health reports `28e7c81fbd00958be6be8e81b56f8f306a1c4c9d`, matching local/main and origin/main at implementation push. Authenticated Scenario 4 passes 2/2 against production (Chromium and Firefox); screenshots were visually inspected locally. This subsequent documentation-only checkpoint records that evidence.
- Follow-up verification: six focused schedule tests pass; changed-file lint is clean; production build and explicit artifact secret scan pass. Serial suite: 463 tests, 454 passed, one live database request failure, eight environment-gated skips; the failing database file subsequently passed 14/14. Local authenticated Scenario 4 passes in Chromium and Firefox, including 390px reflow, no expandable rows, stage focus tooltip, and Today geometry checked against both the range and rendered month bounds. One Chromium login timeout occurred before the chart and passed on retry. Earlier release acceptance below is historical, not a new full release certification.
...
2. Render one ungrouped project/workstream row with individual stage colors and no expansion controls or aggregate stage-count bar. Overlapping stages staircase downward at full 36px height; expand the containing row, never compress the bars. After a workflow steps down, later stages must stay on that lane or step farther down—never rise. Same-day handoffs are sequential and remain on the same lane. If a terminal bar is too short for its text, render its label in free space immediately to the right; never let outside labels collide with a later/same-date bar. For undated stages, project the configured duration (10 calendar days when missing) immediately after the preceding finish; mark these dates as demo projections in tooltips, never persist them as history. Explicit zero keeps that stage and all successors unscheduled outside the calendar. Stage details remain available on hover/focus. These confirmed follow-ups supersede the earlier expandable rows, compressed concurrent bars, and prohibition on display-only demo estimates. Preserve canonical task access through the chronological list and record pages.
