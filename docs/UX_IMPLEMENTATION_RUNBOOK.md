...
- Git synchronization: the current source checkpoint is `8dcf4fda33ffc6830c4d99e531d84d419da90dff`, with matching local `main`/`origin/main` full SHAs and a clean worktree at ledger update time. This is branch synchronization evidence, not a verified tagged-release claim.
- Deployment and production smoke: `https://permit-tracker-iota.vercel.app/api/health` reports status `ok`, environment `production`, and deployed commit `8dcf4fda33ffc6830c4d99e531d84d419da90dff` once this documentation checkpoint is deployed. Deployed Chromium and Firefox runs pass the stage-first shell schedule scenario and the two-user ownership collision scenario against source checkpoint `625f67945a2de2b38136914d44803e65c2317350`; the documentation-only deployment smoke remains to be rerun. The guide is absent and the shell/footer health behavior is covered by browser and source tests.
