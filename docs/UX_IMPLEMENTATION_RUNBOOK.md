...
| R2 — authoritative assignment and Take Ownership | Verified | Two authenticated workers claiming one record, persisted conflict result, assignment/audit/self-claim behavior/queue read-back; distinct-recipient assignment notification read-back |
| R3 — shell and navigation | Verified | Deployed Chromium and Firefox Scenario 4/6 pass: stationary header/sidebar, main-pane scroll ownership, stage-first schedule shell, Back/Forward queue state and focus restoration, mobile drawer Escape/focus return, and 195 CSS-pixel reflow equivalent to 200% zoom without horizontal overflow |
| R4 — real-stage schedule | Verified | Deployed Chromium and Firefox Scenario 4 pass on `83d07ef`; live tagged records contain four five-stage workstreams and 13 persisted stage runs, including two concurrent active Utility stages; source gate covers unscheduled/canonical destinations; seed succeeds twice with stable counts and no duplicate notification dedupe keys |
| CP1–2 — shared truth and lifecycle | Implemented—unverified | Cross-user RFI/clarification stories pass in Chromium and Firefox; counts-versus-rows, completion downstream effects, and the full “complete” continuation remain |
| CP3–6 — routes, inbox, customer, project | Implemented—unverified | Scenarios 1–6 cover fresh-context intake, RFI, identity, shell, assignment, and history; complete all-entry-point agreement, customer-obligation counts, and project completion effects |
| CP7 — schedule | Implemented—unverified | R4 is verified; the remaining CP7 gate is persisted progress/schedule consequence proof across the complete project story |
| CP8 — administration | Verified | Deployed Chromium and Firefox Scenario 9: non-admin editor denial; admin typed task correction requires a reason, persists through audited RPC, refreshes, reads back, and restores the demo task |
| CP9 — workflow authoring | Verified | Authenticated live draft/publish/new-instance/version-pinning story completed through v4; the executable-owner successor passed server validation, a new workstream completed all five stages with persisted stage runs, and prior v1/v2/v3 instances remained pinned to their original versions; browser authoring coverage exercised save/reopen/validation/publish |
| CP10 — usability | Implemented—unverified | R3 and Scenario 6 cover shell, focus, mobile drawer, and 200% zoom-equivalent layout; Scenario 10 now passes in Chromium and Firefox at 390×844 with keyboard and touch navigation; full keyboard/touch walkthroughs and final responsive/accessibility review remain |
| Cross-cutting release gates | Open | Complete exact-document, notifications, coordination, authorization, workflow, two-browser, deployment, and two-consecutive final review evidence |
...
| Supabase migration consistency | Verified | `npx supabase migration list --linked` reports all 65 local/remote versions equal through `20260909170000`; linked `db push --dry-run` reports `upToDate: true` with no pending migrations, seeds, or roles |

### Evidence recorded during the current implementation wave

- Source and focused behavior: the identity-safe claim, staff-operator intake, assignment-label, stage-run hydration, parallel-stage projection, linked-request blocker projection, shell, canonical schedule-link, phase-deep-link, RFI response projection, admin correction, workflow stage-contract, owner validation, mobile navigation, and footer-health tests pass. The current serial full suite completed with **458 tests, 450 passed, 8 skipped, 0 failed**. Skips remain unaccepted environment-gated cases.
- Release checks: current serial full suite, full ESLint command, `npx vinext build`, and explicit `node scripts/scan-build-secrets.mjs` all pass. ESLint reports warnings only (299 existing warnings, 0 errors); the build reports non-blocking large-chunk warnings.
...
- R2 collision evidence: two independently authenticated eligible members, Maya Chen and Gwynne Shotwell, claimed task `task-6cd5d799-3016-4ea9-a1d0-8ef8b8eea61d` concurrently. Exactly one returned `claimed`, the other returned `conflict` naming Gwynne, persisted read-back matched Gwynne, and a `ticket_claimed` audit row identified Gwynne. The task assignment was restored to its original unassigned state after the probe; browser collision proof is also covered by Scenario 5 below.
- R2 browser and notification evidence: deployed Chromium and Firefox Scenario 5 created tagged request `customer-request-2344c7b8-acee-4f82-ae54-0e82484e2a2c` / `PATH-2026-DF731B25`, Sarah Johnson claimed it, Joe Skaggs' stale authenticated claim returned the visible conflict, and a fresh Sarah session rendered Sarah's name with no UUID. Read-back preserved Alex's submitter `50e34731-7c7e-4205-b2d7-7976db33e333`, Sarah's assignee `031dc622-0885-42bb-9c84-1f9b6cb18a1d`, the assignment audit, and `triaged` state. Self-claim correctly emitted no self-notification. A distinct-recipient assignment probe then assigned task `task-6cd5d799-3016-4ea9-a1d0-8ef8b8eea61d` from Maya Chen to Gwynne Shotwell, read back notification `e4a9f1c9-9e6e-47a8-9505-7a3547a6b9ed` for Gwynne, and restored the task to its original unassigned state.
- R3 shell and navigation evidence: deployed Chromium and Firefox Scenario 4 and Scenario 6 pass; the latest Scenario 4 rerun is on `83d07ef61d8c20a681986d52b22a58fe18eaea52`. Scenario 4 read back the stage-first schedule, stationary header, main-pane scroll ownership, no internal schedule overflow, Today at 30%, mobile chronological schedule, and no 390px overflow. Scenario 6 restored My Work search/tab, main scroll position, and navigation focus across Back/Forward; verified mobile drawer Escape/focus return; and verified no document or main-pane horizontal overflow at a 195 CSS-pixel layout viewport, the responsive equivalent of 200% zoom at 390px. The responsive fix removes the global 320px body minimum, stacks narrow queue controls, and wraps long record IDs without hiding identity.
- R4 schedule and persisted blocker evidence: migrations `20260909100000_persist_workstream_block_clock.sql`, `20260909110000_allow_target_group_coordination_response.sql`, and `20260909120000_admin_task_correction_rpc.sql` are applied remotely. The tagged demo seed now records four workstreams on one published five-stage version, 13 durable `stage_runs` rows, a blocked Coast dependency, completed history, and two concurrent active Utility stages. The seed ran twice successfully with stable tagged counts and no duplicate notification dedupe keys. Deployed Chromium and Firefox Scenario 4 passed against `83d07ef61d8c20a681986d52b22a58fe18eaea52`, reading the stage-first schedule, main-pane scroll ownership, Today at 30%, both Utility active stages, square 36px stage blocks, no internal schedule overflow, and the mobile chronological list. Scenario 7 still proves persisted block/unblock after refresh.
- CP8 administrative correction evidence: deployed Chromium and Firefox Scenario 9 passed after `rpc_admin_update_task` was applied. Sarah’s direct admin route was denied; Joe’s typed task edit required a non-empty reason, persisted through the audited RPC, produced an `administrative_correction` audit with Joe as actor, read back after reload, and restored `PATH-DEMO-T-COAST-001` to its blocked demo state.
- CP10 keyboard/touch evidence: production Chromium and Firefox Scenario 10 passed at deployed application checkpoint `04796c0486d3429dbf5c8a31a604d0fa7116c72b` in a 390×844 touch-capable context. Keyboard Enter opened the drawer and selected Team Work; touch opened the drawer and selected My Work; the main content remained the vertical scroll owner. Full customer/staff/admin keyboard walkthroughs and final accessibility review remain open.
- CP1–2 lifecycle evidence: deployed Chromium and Firefox Scenario 2 passed after fresh Jordan reviewer, Alex applicant, and Jordan reviewer contexts. Jordan issued a new RFI, Alex submitted a response, a fresh Jordan session read the exact response, and acceptance/resume completed through the persisted action boundary. Scenario 1 also passed in both browsers with an isolated Alex submission and fresh Sarah intake read-back. The live v4 completion probe advanced a new workstream through all five stages with five persisted stage runs and a final complete state. A fresh authenticated production browser read matched the project database: `2/26 complete` and Work tab `26`; browser-level counts-versus-rows for the project summary are now proven, while the full customer/staff completion story remains a final release review item.
- CP9 workflow evidence: the live draft RPC was first found to clone legacy `workflow_stages`, producing non-canonical owner codes in a newly published v2. A forward migration, `20260909140000_preserve_published_workflow_stage_contract.sql`, now copies the authoritative `workflow_version_stages` contract, including owners, assignment-group defaults, RFI/hold behavior, dependencies, and tasks, with legacy rows retained only as fallback. A live owner-contract check then found that historical `PATH` display codes were not executable organization codes. Migrations `20260909150000_validate_workflow_owner_contract.sql`, `20260909160000_fix_workflow_owner_id_cast.sql`, and `20260909170000_fix_workflow_group_id_cast.sql` normalize invalid legacy owners to active `STATEPO` in new drafts and reject unknown/inactive organizations or groups during validation. An authenticated Joe session created and published v4 from the retired v3 source; v4 contains `STATEPO`, `SPACEPORT`, `LA-PROJECTS`, `STATEPO`, `STATEPO`, validated with `{ valid: true, errors: [] }`, and is active. An authenticated Alex submission `PATH-CP9-REQ-20260909-V4` followed by Joe triage created `PATH-CP9-WS-20260909-V4` pinned to v4. Joe completed all five stages through the authenticated RPC; refresh/read-back showed five completed `stage_runs`, four completed stage-action tasks, and the workstream at `complete` with an actual completion date. Existing Coast and earlier CP9 instances remain pinned to their original versions, proving version pinning. The authenticated browser authoring flow exercised draft creation, stage edit, save/reopen, invalid-duration rejection, correction, and publish; the forward repairs did not edit published history.
- Exact document evidence: deployed Chromium and Firefox `tests/e2e/document-management.spec.ts` passed 4/4 at application checkpoint `04796c0486d3429dbf5c8a31a604d0fa7116c72b`. The browser uploaded a tagged non-empty file, read the immutable version ledger, downloaded the exact version, compared the downloaded bytes with the original buffer, and verified the seeded demo PDF begins with `%PDF-`. Cleanup removed the temporary version and Storage object.
- Live demo workflow: the tagged demo workstreams retain five distinct stages in the published workflow. `stage_runs` hydration is wired, preserves unavailable history rather than inventing completion dates, and now maps multiple persisted active runs to simultaneous current stages.
- Migration reconciliation: the genuinely remote-only SQL statements were recovered from `supabase_migrations.schema_migrations`; locally mis-timestamped files were aligned to genuine applied versions. Normalized statement hashes match the live records. The missing live `rpc_create_workstream_from_request` definition was restored through `20260909130000_restore_workstream_creation_rpc.sql`; the workflow draft, owner normalization, and strict validation boundaries were added through `20260909140000`–`20260909170000`. The linked migration list now reports **65/65 local/remote matches through `20260909170000`**, and the linked dry-run is clean with `upToDate: true` and no pending migrations, seeds, or roles.
- Git synchronization: application/test checkpoint `04796c0486d3429dbf5c8a31a604d0fa7116c72b` is pushed through `main`; full local/remote SHA equality and a clean worktree were confirmed. This is branch synchronization evidence, not a verified tagged-release claim.
- Deployment and production smoke: production health reports application checkpoint `04796c0486d3429dbf5c8a31a604d0fa7116c72b`. The current deployed persistence suite passed **20/20** across Chromium and Firefox, including Scenario 10 keyboard/touch coverage; the current deployed document suite passed **4/4** across Chromium and Firefox. The guide is absent and the shell/footer health behavior is covered by browser and source tests.
- Still open: complete end-to-end customer/staff/admin journey coverage beyond the currently exercised scenarios; browser-level completion downstream effects beyond the authenticated database read-back; full keyboard/touch and accessibility review beyond the exercised paths; independent final notification/coordination cross-checks where not covered by the current suite; and two consecutive final user-story/consistency review passes. No release acceptance claim is made from source, RPC, partial live evidence, or branch synchronization alone.
