...
| R4 — real-stage schedule | Implemented—unverified | September 9 flat-row revision supersedes expandable stages/tasks. Focused tests and local authenticated Chrome/Firefox Scenario 4 pass; deployed smoke remains pending. One project/workstream row contains individually colored stages, with concurrent intervals sharing the track and undated stages explicitly outside the calendar. Today now moves with the actual date range, not a hardcoded marker over unrelated dates. |
