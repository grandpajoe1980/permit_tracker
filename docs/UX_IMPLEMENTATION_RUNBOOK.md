...
- Git synchronization: application checkpoint `625f67945a2de2b38136914d44803e65c2317350` and each follow-up checkpoint were pushed through `main`; the current local `main` and `origin/main` full SHAs are equal at `dcf49db36ffffa5d65479b1ee0290580fc37ff82` with a clean worktree before this documentation update. This is branch synchronization evidence, not a verified tagged-release claim.
- Deployment and production smoke: production health reports `dcf49db36ffffa5d65479b1ee0290580fc37ff82`; deployed Chromium and Firefox Scenario 4 and Scenario 6 pass on that SHA. The guide is absent and the shell/footer health behavior is covered by browser and source tests.
