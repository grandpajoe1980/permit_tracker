...
|---|---|---|
| R0 — baseline and evidence ledger | Verified | Project `zomzacaxwqfwjstkxbpv`, production app `cd3e72227abe57e4517297ef66b8c3ec8428b621`, linked migration parity, tagged personas, and browser environments are recorded below. |
...
| Cross-cutting release gates | Implemented—unverified | Current tests and live checks pass. Final status remains unverified until this evidence ledger update is committed, deployed, and the two final review passes below find no actionable in-scope defect. |
...
- Current deployment: production health at `https://permit-tracker-iota.vercel.app/api/health?sha=cd3e722` reports status `ok`, environment `production`, and the full application SHA `cd3e72227abe57e4517297ef66b8c3ec8428b621`. No Git tag points at this SHA; synchronization is not described as a tagged release.
...
- Exact document evidence: current production `tests/e2e/document-management.spec.ts` passes **2/2 Chromium and 2/2 Firefox**. The browser uploads a tagged non-empty file, reads immutable metadata, downloads the exact Storage object, compares bytes/SHA-256, and verifies the seeded demo PDF; cleanup removes only the temporary tagged object/version.
...
The guide requirement formerly listed under CP10 is superseded by R3. PATH must not ship a replacement welcome panel or replay control. The only remaining release condition is the final commit/deployment synchronization followed by two consecutive user-story and consistency review passes with no actionable in-scope defect.
