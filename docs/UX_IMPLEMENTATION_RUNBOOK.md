...
| R4 — real-stage schedule | Implemented—unverified | September 9 lane-direction follow-up: a workflow never rises after stepping down; same-day handoffs continue right on the same lane, while actual duration overlaps staircase lower. Short terminal bars can place one readable label in free space to their right; intermediate/same-date successors reserve that space. Focused tests and local Chrome/Firefox Scenario 4 pass; deployed smoke pending. |
