...
describe("Challenger 1: Production Worker HTTP Routing Adversarial Tests", () => {
  test("Production Worker HTTP Route Permutations & SSR Delivery", async () => {
    ...
    try {
      ...
      assert.ok(
        nonWsHtml.includes("Workstream not found") ||
          nonWsHtml.includes("Sign in required") ||
          nonWsHtml.includes("WS-NONEXISTENT"),
        "Nonexistent workstream route must render a graceful UI state or preserve the route identity for client hydration"
      );
      ...
      assert.ok(
        nonPrjHtml.includes("Project not found") ||
          nonPrjHtml.includes("Sign in required") ||
          nonPrjHtml.includes("PRJ-DOES-NOT-EXIST"),
        "Nonexistent project route must render a graceful UI state or preserve the route identity for client hydration"
      );
