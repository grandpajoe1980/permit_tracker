...
Provide the Supabase URL, browser-safe key, and configured demo accounts in the test environment. Install the Chromium test browser with `npx playwright install chromium` if it is not already available. The command fails before opening a browser if either key group is missing. It runs Chromium scenarios against the test site and its database; use an isolated demo project because these scenarios create records and retain uniquely tagged immutable document versions as audit evidence. No service-role key is required by this browser suite. See [the Playwright handoff](docs/testing/playwright-handoff.md) for the full scenario contract.
