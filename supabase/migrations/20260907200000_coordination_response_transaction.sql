-- S5: make interagency responses server-owned and independently auditable.
-- A response changes only the coordination request. Dependency/blocker release
-- remains a separate, explicit workstream action.
...
begin
  ...
  -- A response is made by the target agency, a project dispatcher, or a
  -- system administrator. It never relies on a display-name parameter.
  ...
  insert into public.audit_events (
    ...
    'response_recorded',
    v_old_status,
    p_status, trim(p_response_summary), v_workstream.project_id::text,
    v_now, v_now
  );

  -- Notify the requesting agency's configured assignment members. No
  -- dependency or workstream state is changed here.
