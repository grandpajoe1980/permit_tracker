-- Keep published workflow instances executable without mutating their history.
-- Older demo versions used PATH as a display label rather than a real
-- organization code. New drafts normalize that legacy value to the seeded
-- STATEPO project-office organization, while validation rejects any other
-- unknown or inactive owner/group before publication.
...
begin
  ...
  insert into public.workflow_versions (
    ...
    v_draft_id,
    v_source.workflow_id,
    v_next_number,
    'v' || v_next_number::text,
  ...
  if exists (
    ...
    where workflow_version_id = p_source_version_id
  ) then
    ...
    select
      v_draft_id || '-' || s.stage_key,
      v_draft_id,
      s.stage_key,
      s.sequence_order,
      s.label,
      s.internal_description,
      s.customer_visibility_label,
      case when owner.normalized_code = s.responsible_org_code
        then s.responsible_org_id
        else owner.organization_id
      end,
      owner.normalized_code,
      s.responsible_unit_name,
      s.target_duration_days,
      s.minimum_statutory_days,
      s.required_inputs,
      s.completion_requirements,
      s.permitted_transitions,
      s.can_run_in_parallel,
      s.is_milestone_gate,
      s.external_filing_url,
      s.legal_authority_citation,
      s.default_assignment_group_id,
    ...
    select
      ...
      coalesce(s.service_target_days, 0),
      coalesce(s.minimum_processing_days, 0),
      coalesce(s.required_documents, '[]'::jsonb),
      '[]'::jsonb,
      coalesce(s.allowed_transitions, '[]'::jsonb),
      false,
      false
...
begin
  ...
  where s.workflow_version_id = p_version_id
    ...
    and not exists (
      ...
      where g.id = s.default_assignment_group_id and g.active
    );
