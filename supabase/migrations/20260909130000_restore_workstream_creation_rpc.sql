-- Restore the authenticated, audited workstream-creation boundary.
--
-- The historical migration is present in the local/remote ledger, but the
-- live public function definition was absent.  Keep this as a forward repair:
-- customer-request triage must remain an authenticated RPC, not a direct
-- client insert or a service-role-only test shortcut.
...
begin
  ...
  if v_request.status = 'draft' then
    raise exception 'draft customer requests cannot be triaged';
  end if;
  ...
  insert into public.workstreams (
    ...
    coalesce(v_stage.label, 'Request intake'),
    'running',
    'Running (Request intake)',
    'green',
    'On Track',
    current_date + 30,
    current_date + 30,
    v_now,
    jsonb_build_object(
      'name', 'State Project Concierge',
      'title', 'Project Manager',
      'agency', p_lead_org_name
    ),
    jsonb_build_object(
      'orgCode', p_lead_org_code,
      'orgName', p_lead_org_name
    ),
    '{}',
    v_request.id,
    v_now,
  ...
  set status = 'in_progress',
      related_workstream_id = coalesce(v_request.related_workstream_id, v_workstream.id),
      triaged_at = v_now,
      triaged_by_user_id = auth.uid(),
      triaged_workstream_ids = coalesce(triaged_workstream_ids, '[]'::jsonb)
        || jsonb_build_array(v_workstream.id),
      updated_at = v_now
  where id = v_request.id;

  insert into public.audit_events (
    actor_id, action, resource_type, entity_type, entity_id,
    actor_name, action_type, old_value, new_value, project_id, created_at
  ) values (
    auth.uid(),
    'customer_request_triaged',
    'customer_request',
    'customer_request',
    v_request.id,
