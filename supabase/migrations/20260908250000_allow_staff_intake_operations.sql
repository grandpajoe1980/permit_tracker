-- Allow authenticated non-customer project staff to perform intake operations while preserving project and group scope.
create or replace function app_private.require_project_operator(p_project_ref text)
returns uuid
language plpgsql
security definer
...
begin
  ...
  select p.id, p.customer_organization_id
    into v_project_id, v_customer_org_id
  ...
  if v_project_id is null then
    raise exception 'project not found: %', p_project_ref;
  end if;
  if not app_private.has_project_access(v_project_id) then
    raise exception 'authenticated user cannot access project %', p_project_ref;
  end if;
  if app_private.is_customer_org_member(v_customer_org_id) then
    raise exception 'project staff capability required';
  end if;
  return v_project_id;
end;
$$;

revoke execute on function app_private.require_project_operator(text) from public, anon;
grant execute on function app_private.require_project_operator(text) to authenticated;
...
begin
  ...
  v_project_id := app_private.require_project_operator(v_request.project_id);
