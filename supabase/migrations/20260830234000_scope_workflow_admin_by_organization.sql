-- Wave 3 follow-up: organization administrators may manage workflow metadata
-- for their own organization only. Creating a new organization remains a
-- system-admin operation because it creates a new tenant boundary.
...
drop function if exists app_private.require_workflow_admin();
create or replace function app_private.require_workflow_admin(p_organization_id uuid default null)
returns void
...
begin
  if auth.uid() is null
     or (p_organization_id is null and not (select app_private.is_system_admin()))
     or (p_organization_id is not null and not (select app_private.is_organization_admin(p_organization_id))) then
    raise exception 'workflow administrator capability required for organization %', p_organization_id;
  end if;
end;
$$;
revoke all on function app_private.require_workflow_admin(uuid) from public;
grant execute on function app_private.require_workflow_admin(uuid) to authenticated;
...
for select to authenticated using (
  exists (
    ...
    where v.id = workflow_version_id
      and (select app_private.is_org_member(d.organization_id))
  )
);
...
begin
  ...
  select organization_id into v_organization_id from public.workflow_definitions where id = v_source.workflow_id;
...
declare
  ...
  v_org_uuid uuid;
begin
  select id into v_org_uuid from public.organizations where code = upper(trim(p_responsible_org_code));
  if v_org_uuid is null then raise exception 'responsible organization not found: %', p_responsible_org_code; end if;
  perform app_private.require_workflow_admin(v_org_uuid);
